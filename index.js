...
mongoose
  .connect("mongodb+srv://abhi013:Akbarnama%40123@cluster0.sdboo.mongodb.net/mernapp?retryWrites=true&w=majority", {
...
app.post('/adddoc', upload.single('image'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).send('No file uploaded.');
    }
    console.log('req.file:', req.file); // Check the file object
    console.log('req.body:', req.body); // Check other form data  
    // Parse the employeeDetails from JSON string to an object
    const employeeDetails = JSON.parse(req.body.employeeDetails);

    const { employeeName, employeeId } = employeeDetails;
    const image = req.file.buffer; // Image is stored in buffer format

    // Create a new Image document
    const newImage = new Image({
      employeeName,
      employeeId,
      image, // Assuming your Image model has a field for storing the image buffer
    });

    await newImage.save();

    res.status(201).json({ message: "Doc saved successfully", image: newImage });
  } catch (error) {
    console.log("Error creating document", error);
    res.status(500).json({ message: "Failed to add a document" });
  }
});
...
app.get("/image", async (req, res) => {
  try {
    const employees = await Image.find();
    res.status(200).json(employees);
  } catch (error) {
    res.status(500).json({ message: "Failed to retrieve the employees" });
  }
